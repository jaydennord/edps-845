...
```{r}
# data_freqs <- lapply(data_list, function(d) sapply(d, table))
# OR
...
for(i in names(data_list)){
  data_list[[i]]$schoolid <- PISA$students[row.names(data_list[[i]]), "schoolid"]
}
...
desc <- lapply(data_list, function(d) {
  lapply(d[, -5], tapply, d[, "schoolid"], dstudy)
})
```
...
```{r}
# epmr package already loaded from previous block
# DO THIS!
```
